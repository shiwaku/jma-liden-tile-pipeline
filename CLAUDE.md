...
- **カバレッジの判定に `data/raw/` の有無を使わない。** `archive/manifest.json` の
  ...
  **これは `status.py` だけの話ではない。** `collect.py` の「未取得」判定も
  同じで、raw だけを見ると `data/raw/` が無い環境（CI は毎回まっさら）で
  窓のぶんを丸ごと取り直す。枠番は必ず `slice_day_index()` を使う
  （数え方を変えると1枠ずれて「毎回1スライスだけ取り直す」壊れ方をする）。
